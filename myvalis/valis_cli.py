...
try:
    from valis_alignment_clean import (
        AlignmentConfig, 
...
def align(
    ...
    flip_horizontal: bool = typer.Option(
        True,
        "--flip/--no-flip",
        help="Whether to flip DAPI image horizontally (deprecated, use --flip-axis)"
    ),
    flip_axis: str = typer.Option(
        "y",
        "--flip-axis",
        help="Flip axis: 'none', 'x' (horizontal), or 'y' (vertical)"
    ),
    um_per_pixel: float = typer.Option(
        0.5072,
        "--um-per-pixel",
        min=0.001,
        max=10.0,
        help="Microns per pixel for coordinate conversion"
    ),
    # TSV column options
    ...
    # Overlay options
    save_overlays: bool = typer.Option(
        True,
        "--save-overlays/--no-save-overlays",
        help="Save overlay images to workdir/results/overlays"
    ),
    overlay_alpha: float = typer.Option(
        0.5,
        "--overlay-alpha",
        min=0.0,
        max=1.0,
        help="Alpha for overlay blending (0..1)"
    ),
    # Runtime controls
    timeout_s: Optional[float] = typer.Option(
        None,
        "--timeout",
        help="Timeout in seconds for VALIS registration (default: no timeout)"
    ),
    heartbeat_s: int = typer.Option(
        30,
        "--heartbeat",
        help="Heartbeat interval in seconds to log during registration"
    ),
    retry_on_timeout_cpu: bool = typer.Option(
        True,
        "--retry-on-timeout/--no-retry-on-timeout",
        help="Retry on CPU with safer settings if registration times out"
    ),
    # CLI options
    ...
    config.flip_horizontal = flip_horizontal  # Keep for backward compatibility
    config.flip_axis = flip_axis
    config.microns_per_pixel = um_per_pixel
    config.centroid_x_col = x_column
    config.centroid_y_col = y_column
    config.verbose = verbose
    config.save_overlays = save_overlays
    ...
    # Runtime controls for registration
    config.registration_timeout_s = timeout_s
    config.heartbeat_s = heartbeat_s
    config.retry_on_timeout_cpu = retry_on_timeout_cpu
...
def display_config(config: CLIConfig, dry_run: bool = False):
    ...
    table.add_row("Flip Axis", config.flip_axis)
    table.add_row("Microns/Pixel", f"{config.microns_per_pixel:.4f}")
    table.add_row("X Column", config.centroid_x_col)
    table.add_row("Y Column", config.centroid_y_col)
    table.add_row("Save Overlays", str(getattr(config, 'save_overlays', True)))
    table.add_row("Overlay Alpha", f"{getattr(config, 'overlay_alpha', 0.5):.2f}")
    ...
    # Runtime controls
    table.add_row("Timeout (s)", str(config.registration_timeout_s))
    table.add_row("Heartbeat (s)", str(config.heartbeat_s))
    table.add_row("Retry on Timeout", str(config.retry_on_timeout_cpu))
...
def display_results(results: dict, verbose: bool = False):
    ...
    # Registration error (robust to None)
    err_df = results.get('registration_error', None)
    try:
        import pandas as pd  # local import to avoid top-level dependency if not needed
        ...
        # If anything goes wrong computing error stats, just skip
        pass
    ...
    if verbose:
        if err_df is not None:
            try:
                ...
                console.print(err_df.to_string())
            except Exception:
                pass
